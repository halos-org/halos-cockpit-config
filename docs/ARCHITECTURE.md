...
3. **Debian Package Metadata** - Control files, scripts, and build configuration
4. **Build System** - Docker container and scripts for reproducible builds
5. **Maintainer Scripts** - Pre/post install scripts for safe file management
...
- Provide the correct path-based URL
- Configure card layout and category

### Traefik Reverse Proxy

Traefik provides a path-based redirect for Cockpit:
- `https://{hostname}.local/cockpit/` redirects (302) to Cockpit's native HTTPS on port 9090
